...
Use `website` as the project root, `pnpm run build` as the build command, and `dist` as the output directory. Set `VITE_SITE_URL` in the Vercel project settings when deploying to a custom domain. The included `vercel.json` preserves the `/privacy` and `/support` routes.
